...
def get_adaccounts(user):
    url = 'https://graph.facebook.com/v2.12/' + \
        user['user_id'] + '/adaccounts?access_token=' + \
        user['long_access_token']
...
def nocap_check(db, adaccounts):
    ...
    new_adaccounts = list(set([adaccount['account_id'] for adaccount in adaccounts]
                              ) - set([nocap['adaccount'] for nocap in nocap_list]))
    result = []
...
def get_entities_list(db, user, adaccount, entity):
    ...
    url = 'https://graph.facebook.com/v2.12/' + \
        adaccount['id'] + '/' + entity + \
        's?access_token=' + user['long_access_token']
...
def get_entity_field_values(db, user, adaccount, entity, entity_type):
    ...
    url = 'https://graph.facebook.com/v2.12/' + \
        entity['id'] + '?access_token=' + user['long_access_token']
    headers = {'Content-Type': 'application/json; charset=utf-8',
               'content-encoding': 'gzip'}
    response = requests.get(url, params=params, headers=headers)
    response = response.json()
    ...
    if 'error' in response:
        ...
        if response['error']['code'] == 17:
            ...
            print("reach api limit, wait {} seconds and retry".format(
                TIME['limit_wait_time']))
            time.sleep(TIME['limit_wait_time'])
            response = get_entity_field_values(
                db, user, adaccount, entity, entity_type)
    try:
        return response
    except Exception as e:
        return []
...
def get_entity_insights(user, entity_name, entity, breakdowns):
    ...
    url = 'https://graph.facebook.com/v2.12/' + \
        entity['id'] + '/insights?access_token=' + user['long_access_token']
    headers = {'Content-Type': 'application/json; charset=utf-8',
               'content-encoding': 'gzip'}
    response = requests.get(url, params=params, headers=headers)
    response = response.json()
    ...
    if 'error' in response:
        if response['error']['code'] == 17:
            ...
            print("reach api limit, wait {} seconds and retry".format(
                TIME['limit_wait_time']))
            time.sleep(TIME['limit_wait_time'])
            response['data'] = get_entity_insights(
                user, entity_name, entity, breakdowns)
    try:
        return response['data']
    except Exception as e:
        return []
            ...
            for adaccount in adaccounts:
                ...
                for entity_type in entity_types:
                    ...
                    for entity in entities:
                        ...
                        if insights:
                            update_db(db, 'stats_' + entity_type,
                                      entity_type + '_id', insights, breakdown)
                            time.sleep(TIME['loop_wait_time'])
                            field_values = get_entity_field_values(
                                db, user, adaccount, entity, entity_type)
                            if field_values:
                                update_field_values(
                                    db, user, field_values, breakdown)
